...
{
//    qDebug() << "createEditor";
    Q_UNUSED(option);// Неиспользуемые параметры
//    Q_UNUSED(index);
...
//    qDebug() << "setEditorData";
    ...
    QComboBox *cb = qobject_cast<QComboBox *>(editor);//Получаем выпадающий список
    Q_ASSERT(cb);  //убеждаемся, что он не пустой
    const QString currentText = index.data(Qt::DisplayRole).toString();// Получаем текущее значение для выпадающего списка
    const int cbIndex = cb->findText(currentText);// Находим его в выпадающем списке
    // Если нашли:
    if (cbIndex >= 0)
